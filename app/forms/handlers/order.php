...
function createOrder()
{
    ...
    try {
        ...
        setProductToOrder($orderId, $cart);
...
function  setProductToOrder(int $orderId, array $cart): void
{
...
function minusProductQty(int $id, int $quantity): void
{
    ...
    if (!$query->execute(compact('id', 'quantity'))) {
        throw new Exception("Error while updating product quantity, with product id=$id qty=$quantity");
    }
}
