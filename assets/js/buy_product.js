const selectors = {
    ...
    modal: {
        ...
        additions: {
            ...
            price: '.additional-price .price-addition',
            qty: '.additional-qty',
...
$(document).on('change', `${selectors.modal.form} ${selectors.modal.product.quantity} `, function () {
    const qty = $(this).val()
    const price = parseFloat($(`${selectors.modal.product.price} .price-addition`).text())
    $(selectors.modal.product.total).html(renderPrice((qty * price)))
    calculateTotalPrice()
...
{
    return `$<span class="price-addition">${price.toFixed(2)}</span>`
}

function calculateTotalPrice() {
    let productTotal = parseFloat($(`${selectors.modal.product.total} .price-addition `).text())
    ...
    if (additions.length > 0) {
        additions.toArray().map(function (el) {
            ...
            const total = parseFloat($parent.find(`${selectors.modal.additions.total} .price-addition `).text())
            productTotal += total
        })
    }
